# v7.4.15 Developer Notes

- M BOOST 특화 관리는 `easyPaySpent` 필드 기준으로 변경했다.
- 레거시 `mPoint`, `mPointEarned`, `mPointUsed`는 UI 계산에서 사용하지 않는다.
- 기존 M포인트 값이 있는 경우 `easyPaySpent`가 없을 때만 `mPoint / 0.05`로 1회 환산한다.
- M BOOST 한도 계산은 `MBOOST_EASYPAY_LIMIT = 200000`, `MBOOST_EASYPAY_RATE = 0.05`, `MBOOST_EASYPAY_POINT_CAP = 10000` 기준이다.
- 구독 자동 포함 추정은 `due < todayStart && due <= asOf` 조건으로 제한했다. 기준일이 미래여도 오늘/미래 결제는 반영예정으로 남긴다.
